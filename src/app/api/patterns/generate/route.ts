...
export async function POST() {
  ...
  const { data: decisions } = await supabase.from("decisions").select("*, outcome:outcomes(*)").eq("user_id", user.id).order("created_at", { ascending: false }).limit(30);
  ...
  try {
    ...
    let parsed;
    try {
      const prompt = `You are an investment behavior analyst. Analyze these decisions for an Indian retail investor. Identify exactly 3 behavioral patterns affecting returns. Be specific and data-driven. Format as JSON only: {"patterns":[{"pattern_text":"string","confidence_percent":number,"based_on_decisions":number,"pattern_type":"bias|timing|asset_class|emotion|technical"}]}. Data: ${JSON.stringify(formatted)}`;
    ...
    const toInsert = parsed.patterns.map((p: { pattern_text: string; confidence_percent: number; based_on_decisions: number; pattern_type: string }) => ({ user_id: user.id, ...p }));
